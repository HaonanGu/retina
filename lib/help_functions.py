...
def masks_Unet(masks,num_lesion_class):
    ...
    im_h = masks.shape[2]
    im_w = masks.shape[3]
    masks = np.reshape(masks,(masks.shape[0],1,im_h*im_w))
    new_masks = np.empty((masks.shape[0],im_h*im_w,1+1))

    new_masks[:,:,0:1]=masks[:,0:1,:].transpose(0,2,1)
    #for i in range(4):
    mask0=new_masks[:,:,0]
    ...
    new_masks[:,:,num_lesion_class]=1-(m0.mask)
